...
class PreventEmptySessionCookieIntegrationTest extends TestCase
{
    ...
    public function testEmptySessionDoesNotWriteToRedisAndDeletesCookie(): void
    {
        ...
        set_error_handler(static function (int $severity, string $message): bool {
            ...
            return strpos($message, 'session_destroy') !== false || strpos($message, 'Session object destruction failed') !== false;
        });
        ...
        if (count($headers) === 0) {
            self::markTestIncomplete('Cannot capture Set-Cookie headers in CLI without Xdebug; skipping header verification. Cookie deletion logic is still verified via $_COOKIE assertion.');
        } else {
            $cookieCleared = false;
    ...
    public function testCleanupHandlerRegisteredForNewSession(): void
    {
        ...
        foreach ($testHandler->getRecords() as $record) {
            if ($record['level_name'] === 'DEBUG' && $record['message'] === 'Registered empty session cleanup handler') {
